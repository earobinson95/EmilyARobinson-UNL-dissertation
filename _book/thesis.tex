...
Section \protect\hyperlink{r2d3-with-shiny}{3.2} outlines the development of the you draw it study interactive plots through the use of r2d3 with R shiny applications.
...
Mosteller, Siegel, Trapido, \& Youtz (1981) was replicated as part of the study data collection in order to validate the you draw it method, this will be refered to as Eye Fitting Straight Lines in the Modern Era.
...
\subsubsection{Eye Fitting Straight Lines in the Modern Era}\label{eye-fitting-straight-lines-in-the-modern-era}}

\emph{Algorithm: Linear Data Generation} \texttt{linearDataGen()}
...
\item
  Determine y-intercept at x = 0 from the provided slope and y-intercept at the mean of x (y\_xbar).

  \begin{itemize}
  \tightlist
  \item
    Slope-intercept form: \texttt{y\ -\ y\_xbar\ =\ m(x-xbar)}
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Set constraint of the mean of the first N/3 = 10 errors less than \textbar2*sigma\textbar{}
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \texttt{y\ =\ yintercept\ +\ slope*x\ +\ error}
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \texttt{lm(y\ \textasciitilde{}\ x,\ data\ =\ point\_data)}
  \end{itemize}
\item
  ...
  \tightlist
  \item
    \texttt{y\ =\ yintercepthat\ +\ slopehat*x}
  \end{itemize}
\item
...
\item
  ...
  \item
    x\_range = c(0,20)
  \item
    y\_range = range(all eye fitting data)*c(1.1, 1.1)
  \end{itemize}
\item
...
\subsubsection{Prediction of Exponential Trends}\label{prediction-of-exponential-trends}}
...
\hypertarget{data-driven-documents-setup-with-shiny-r2d3-with-shiny}{%
\subsection{Data Driven Documents Setup with Shiny \{r2d3-with-shiny\}}\label{data-driven-documents-setup-with-shiny-r2d3-with-shiny}}
...
\tightlist
\item
  \url{https://benediktehinger.de/blog/science/scatterplots-regression-lines-and-the-first-principal-component/}
\item
  \protect\hyperlink{d3.js-source-code}{Appendix A}
\end{itemize}
...
\hypertarget{eye-fitting-straight-lines-in-the-modern-era-1}{%
\subsection{Eye Fitting Straight Lines in the Modern Era}\label{eye-fitting-straight-lines-in-the-modern-era-1}}
...
\hypertarget{prediction-of-exponential-trends-1}{%
\subsection{Prediction of Exponential Trends}\label{prediction-of-exponential-trends-1}}
...
\chapter{You draw it D3.js source code}\label{d3.js-source-code}}
...
\CommentTok{\# do not fill the line in (default is filled in black beneath the line)}
\CommentTok{\# stroke provides the color of the line stroke. This is either options.data\_line\_color (not yet defined) or steelblue}
\CommentTok{\# sets the the stroke width (how thick)}
\CommentTok{\# sets how tojoin the lines? and end the lines? rounded.}
\CommentTok{\# Since we are using Object.assign, we are passing the source options.line\_style into the vector of information defined. See top for options list.}
...
\CommentTok{\# provides the data from top}
\CommentTok{\# appends the svg group and moves it to the correct location...}
\CommentTok{\# sets the width and height of the plot}
\CommentTok{\# Since we are using object.assign, the options are passed in as the source, this will "overwrite" any information provided in the options???}
\CommentTok{\# Is state like our wrapper?}
...
\CommentTok{\# option variables are passed into state}
\CommentTok{\# console.log(state.y\_range)}
...
\CommentTok{\# https://rstudio.github.io/r2d3/articles/advanced\_rendering.html}
...
\CommentTok{\# is this like our bounds??? or does this have to do with scales?}
\CommentTok{\# https://rstudio.github.io/r2d3/articles/advanced\_rendering.html}
\CommentTok{\# redraws plot as you resize your browser window }
\CommentTok{\# (box has changed size that we did not do on code end)}
...
\CommentTok{\# set up scales \& draw true line if we decide to do that.}
...
\NormalTok{  \}}
  ...
  \CommentTok{\# Cover hidden portion with a rectangle}
  ...
  \ControlFlowTok{if}\NormalTok{(reset)\{}
    \CommentTok{\# start with the svg object provided by r2d3}
    \CommentTok{\# multiassign where setup\_drawable\_points is grabbing the parameters it needs from state: data, freedraw and draw\_start}
...
\NormalTok{  \};}
  ...
  \CommentTok{\# passes drawn points to shiny when click done}
  \CommentTok{\# as long as there are dots (missing spaces) it isn\textquotesingle{}t going to pass data back.}
...
\NormalTok{    const line\_status }\OtherTok{=} \FunctionTok{get\_user\_line\_status}\NormalTok{(state);}
    ...
    \ControlFlowTok{if}\NormalTok{(line\_status }\SpecialCharTok{==}\ErrorTok{=} \StringTok{\textquotesingle{}done\textquotesingle{}}\NormalTok{)\{}
      ...
      \SpecialCharTok{/}\ErrorTok{/}\ControlFlowTok{if}\NormalTok{(state.show\_finished) }\FunctionTok{line\_hider.reveal}\NormalTok{();}
      \SpecialCharTok{/}\ErrorTok{/}\ControlFlowTok{if}\NormalTok{(}\SpecialCharTok{!}\NormalTok{state.free\_draw)  }\FunctionTok{line\_hider.reveal}\NormalTok{();}
...
\CommentTok{\# from state we need drawable\_points {-} from setup\_drawable\_points() function that modifies state (get all x points bigger than or equal to draw\_start and set up with a null), pin\_start, and free\_draw parameters}
\CommentTok{\# drag\_x, drag\_y come from on\_drag() function}
...
\ControlFlowTok{function} \FunctionTok{setup\_draw\_watcher}\NormalTok{(svg, scales, on\_drag, on\_end)\{}
  ...
  \CommentTok{\# could have called drag\_watcher whatever we wanted}
  \CommentTok{\# .at is space it begins watching}
  \CommentTok{\# .call is "do something" d3.drag() is the mouse action puts listeners to react to user input.}
...
\NormalTok{  const yAxisGrid }\OtherTok{=} \FunctionTok{d3.axisLeft}\NormalTok{()}\FunctionTok{.scale}\NormalTok{(y)}\FunctionTok{.tickSize}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{w)}\FunctionTok{.tickFormat}\NormalTok{(}\StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{);}
  ...
  \CommentTok{\# Remove all grid related things first}
  \CommentTok{\# remove everything when you convert from linear and log and vise versa.}
  \CommentTok{\# want html element \textless{}g\textgreater{} that has class axis{-}grid}
