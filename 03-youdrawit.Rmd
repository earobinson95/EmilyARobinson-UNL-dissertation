...
In June 2020, the NY Times released a front page displaying figures that represent each of the 100,000 lives lost from the coronavirus pandemic until this point in time [@NYTrememberinglives]; this visualization was meant to bring about a visceral reaction and resonate with readers. 
...
<!-- https://martech.zone/vecteezy-svg-editor-online/ -->
...
```{r ols-vs-pca-example, fig.height = 4, fig.width = 4, fig.cap="OLS vs PCA Regression Lines", message=FALSE, warning=FALSE, out.width="65%"}
library(ggplot2)
library(magrittr)
library(plyr)
...
ggplot(aes(x1,x2))+
  geom_point()+
  geom_abline(data=slopeData,aes(slope = slope,intercept=0,color=type, linetype=type))+
...
```{r eyefitting-lmer-residualplots, fig.height = 6, fig.width = 6, fig.cap = "Eye Fitting Straight Lines in the Modern Era LMM results", out.width="80%"}
...
```{r eyefitting-gamm-residualplots, fig.height = 6, fig.width = 6, fig.cap = "Eye Fitting Straight Lines in the Modern Era GAMM results", out.width="80%"}
...
```{r exponential-yloess-spaghetti-plot, fig.height = 6, fig.width = 6, out.width = "75%", fig.cap = "Exponential Prediction Spaghetti Plot"}
...
```{r exponential-prediction-gamm-preds, fig.height = 6, fig.width = 6, out.width = "75%", fig.cap = "Exponential Prediction GAMM Results"}
