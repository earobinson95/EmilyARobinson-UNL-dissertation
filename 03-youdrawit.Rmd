...
The x-axis is the $\log_{1.5}$ dose level for four dose levels (for example, doses 4, 6, 9, and 13 correspond correspond to equally spaced values on a logarithmic scale - 0, 1, 2, and 3) and the y-axis is the corresponding probit response as caluclated in @finney1948table; circles correspond to the test preparation method while the crosses correspond to the standard preparation method .
...
In June 2020, the New York Times released a front page displaying figures that represent each of the 100,000 lives lost from the coronavirus pandemic until this point in time [@NYTrememberinglives]; this visualization was meant to bring about a visceral reaction and resonate with readers. 
...
Data Driven Documents depend on Extensible Markup Langage (XML) to generate graphics and images by binding objects and layers to the plotting area as Scalable Vector Graphics (SVG) in order to preserve the shapes rather than the pixels \pcref{fig:raster-vs-vector} [@raster_vs_svg]
...
[Appendix B](#exponential-prediction-plots) displays visual examples of all 8 interactive plots.
...
exp_data %>%
  ...
  scale_y_continuous("y")
```
Allowing for flexibility, the bam function in the mgcv package [@mgcv1; @mgcv2; @mgcv3; @mgcv4; @mgcv5] is used to fit a GAMM to estimate trends of vertical residuals from the participant drawn line in relation to the NLS fitted values ($e_{ijklm,NLS} = y_{ijklm,drawn} - \hat y_{ijklm,NLS}$) across the range of $x$. 
...
grid_data.exp.reduced %>%
  ...
  geom_line(data = exp_data, aes(x = x, y = residualdrawn, group = plotID), alpha = 0.05) +
