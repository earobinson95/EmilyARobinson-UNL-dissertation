...
Mosteller et al. (1981) was replicated as part of the study data collection in order to validate you draw it as a method for testing graphics, referred to as Eye Fitting Straight Lines in the Modern Era. 
...
\rh{you have xijk and xk -> a little confusing. you mention "domain of "x". what do you mean by that?}

```{r ols-vs-pca-example, fig.height = 8, fig.width = 8, fig.cap="OLS vs PCA Regression Lines", message=FALSE, warning=FALSE, out.width="100%"}
library(ggplot2)
library(magrittr)
library(plyr)
...
```{r eyefitting-lmer-residualplots, fig.height = 12, fig.width = 12, out.width = "100%", fig.cap = "Eye Fitting Straight Lines in the Modern Era LMM results"}
...
```{r eyefitting-gamm-residualplots, fig.height = 12, fig.width = 12, out.width = "100%", fig.cap = "Eye Fitting Straight Lines in the Modern Era GAMM results"}
...
A nonlinear least squares regression is then fit to the simulated points in order to obtain the best fit line and fitted values in 0.25 increments across the domain, $(x_k, \hat y_{k,NLS}), k = 1, ..., 4 x_{max} +1$.
The data simulation function then outputs a list of point data and line data both indicating the parameter identification, x value, and corresponding simulated or fitted y value.
...
```{r exponential-yloess-spaghetti-plot, fig.height = 12, fig.width = 12, out.width = "100%", fig.cap = "Exponential Prediction Spaghetti Plot"}
...
exp_data %>%
  ...
  scale_y_continuous("y")
```
Allowing for flexibility, the bam function in the mgcv package [@mgcv1; @mgcv2; @mgcv3; @mgcv4; @mgcv5] is used to fit a GAMM to estimate trends of vertical residuals from the participant drawn line in relation to the NLS fitted values ($e_{ijklm,NLS} = y_{ijklm,drawn} - \hat y_{ijklm,NLS}$) across the domain of $x$. 
...
```{r exponential-prediction-gamm-preds, fig.height = 12, fig.width = 12, out.width = "100%", fig.cap = "Exponential Prediction GAMM Results"}
