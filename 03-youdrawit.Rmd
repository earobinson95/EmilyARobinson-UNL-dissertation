...
```{r youdrawit-setup, message=FALSE, warning=FALSE, echo = F, cache = F}
library(tidyverse)
library(patchwork)
library(here)
library(readr)
...
```{r eyefitting-data, message=FALSE, warning=FALSE, cache = F}
...
### \svp{A Review of Regression and Prediction} Methodology
...
For readers familiar with R, it is notable to consider D3 in JavaScript equivalent to the `ggplot2` package in R [@ggplot2]. \svp{eek, that makes me squirm. D3 is not a grammar-of-graphics approach. It is a nice plotting framework though.}
...
```{r exponential-yloess-spaghetti-plot, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 9, out.width = "100%", fig.cap = "Exponential Prediction Spaghetti Plot", cache = F}
