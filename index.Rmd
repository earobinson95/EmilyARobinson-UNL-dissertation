...
```{r statistical-atlas-state-population, fig.cap = "Statistical Atlas 1870 state population", out.width="90%"}
...
```{r ATandT-revenue, fig.cap = "AT\\&T utilization of assets", out.width="90%"}
...
https://srvanderplas.github.io/Perception-of-Log-Scales/estimation-development/questions-and-data.html
...
@joram2005children was interested in the relationship among strategy use and accuracy of students’ representations of standard measurement units and measurement accuracy. Students were asked to estimate the lengths of two objects. After estimation, students were asked:
...
Results: students who used a reference point had a more accurate representation of standard units and estimates of length than students who did not use a reference point.

@joram1998measurement

@jones2012students

+  examined students’ accuracy of measurement estimation for linear distances, different units of measure (metric vs English / novel units), task context, and the relationship between accuracy estimation and logical thinking 
+ Students were also less accurate in estimating in metric units as compared to English or novel units.
Estimation accuracy was dependent on the task context
+ Estimation accuracy was dependent on the task context.
+ In order to interact effectively with our environment it is essential to possess an intuitive grasp of both dimension and scale and to be able to manipulate such information. Estimation, approximating and measuring are all components of such intuition (Forrester, Latham, & Shire, 1990, p. 283)

@albarracin2014devising

@hogan2003quantitative

+ The mathematics education literature refers to 3 types of quantitative estimation skill: 
  + **numerosity:** refers to estimating the number of items in an array, usually presented in such
a way as to preclude exact counting of the items
  + **measurement:** requires the participant to provide estimates of length, height, weight, liquid capacity, and similar measures, usually for common objects in the environment
  + **computational estimation:** refers to providing estimated answers to computations such as 328 + 719, 4269 ÷ 22, and .19 × 1.87. The tasks may be presented either in algorithmic form (e.g., 26 × 419 = ___) or in simple word problems
+ Quantitative estimation has a much longer history in the psychometric literature than in the mathematics education literature. Quantitative estimation tasks appeared in some of the earliest and most famous psychometric studies of mental abilities.

### Anchoring and Rounding Errors
...
```{r log-scale-comic, include = T, fig.cap = "Usefulness of the log scale in science", out.width="100%"}
