...
Misleading charts might have (1) bad form such as 3D pie charts or an incompatiable plot for the data \pcref{fig:misleading-graphics-form} [@reddit_dataisugly; @lowndes2017our], (2) include too much "chartjunk", resulting in clutter or displaying useless data \pcref{fig:misleading-graphics-chartjunk} [@graphcrimes_2022; @wikipedia_x86_64], or (3) have bad axes such as a mismatch between scale and context or just plain bad math \pcref{fig:misleading-graphics-bad-axes} [@villa_2022; @lauren_2018]. \cref{fig:misleading-graphics-just-bad} demonstrates how a chart can be misleading in more than one way; the map of pet ownership violates guidelines by mapping too many variables to visual encodings which results in clutter and bad form [@reddit_CrappyDesign].
...
A major issue we encountered in the creation of COVID plots was how to display data from a wide range of values.
