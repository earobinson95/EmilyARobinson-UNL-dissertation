...
```{r 91divoc-cases-july2021, fig.cap = "91-DOVIC new daily case counts as of July 2021", out.width="75%"}
knitr::include_graphics("images/91dovic-cases-july2021.png")
```

```{r covid19-summer2020-risk-map, fig.cap = "COVID-19 risk level map as of July 2020", out.width="75%"}
knitr::include_graphics("images/covid19-summer2020-risk-map.png")
```

```{r covid19-datawrapper-views-july2020, fig.cap = "Datawrapper daily chart views during COVID-19", out.width="75%"}
...
Misleading charts might have (1) bad form such as 3D pie charts or a plot type that is unsuitable for the type of data \pcref{fig:misleading-graphics-form} [@reddit_dataisugly; @lowndes2017our], (2) include too much "chartjunk", resulting in clutter or displaying useless data \pcref{fig:misleading-graphics-chartjunk} [@graphcrimes_2022; @wikipedia_x86_64], or (3) have bad axes such as a mismatch between scale and context or just plain bad math \pcref{fig:misleading-graphics-bad-axes} [@villa_2022; @lauren_2018].
...
```{r misleading-graphics-form, fig.show = 'hold', fig.aligh = 'center', out.width = '49%', fig.cap = "Misleading graphical form examples"}
...
```{r misleading-graphics-chartjunk, fig.show = 'hold', fig.aligh = 'center', out.width = '49%', fig.cap = "Misleading 'chartjunk' examples"}
...
```{r misleading-graphics-bad-axes, fig.show = 'hold', fig.aligh = 'center', out.width = '49%', fig.cap = "Misleading bad axes"}
...
```{r misleading-graphics-just-bad, fig.align = 'center', out.width="50%", fig.cap = "Misleading graphic multiple violations example"}
...
```{r covid-19-reporting, fig.cap = "COVID-19 reporting", out.width="100%"}
...
Many efforts have been made to provide \svp{frameworks and classification systems} for graphical designs; \svp{the most useful for our purposes is} Wilkinson's Grammar of Graphics [@wilkinson2013grammar].
...
Software, such as `ggplot2` [@ggplot2], aims to implement the framework of creating charts and graphics \svp{using the layered framework of the grammar of graphics}. 
...
\svp{Graphical guidelines/rules to discuss here: the graphics group ASME design guidelines. Tufte's "chartjunk". Hadley's 100 years of graphical criticism (and papers cited in there).}

\svp{Helpful suggestions for creating "good" graphics are extremely common (and sometimes conflicting). In the absence of an underlying understanding of how graphics are perceived and used, these suggestions are largely ineffective and sometimes harmful. It is essential to have guidelines established through careful experimentation combined with an understanding of the perceptual and cognitive processes involved in the use of statistical graphics. }
