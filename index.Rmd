...
Results of the study lead the authors to argue for the use of circle diagrams to show component parts based on both accuracy and speed.
...
Participants were presented a cost volume profit graph \pcref{amer-poggendorff-illusion} with two crossing lines (revenue and cost) and asked to view three values: (1) the amount of total revenues on the ordinate corresponding to the endpoint of the total-revenue line plotted on the graph (2) the amount of total costs on the ordinate corresponding to the endpoint of the total-cost line plotted on the graph and (3) the amount of costs/revenues on the ordinate at the break even point—the point where the two lines cross.
...
Participants were strictly informed that the experiment was designed to test the ability of individuals to "read" or "decode" statistical maps and asked to write down their estimate of the murder rate as accurately as possible along- side the 24 named states. 
...
```{r amer-poggendorff-illusion, fig.cap = "Amer (2005) cost volume profit graph", out.width="75%"}
knitr::include_graphics("images/amer-poggendorff-illusion.png")
```

```{r framed-murder-rate-map, fig.cap = "Dunn (1988) maps", out.width="75%"}
...
Supported by the grammar of graphics, a data plots can be characterized as a statistic defined as, "a functional mapping of a variable or set of variables" [@vanderplas_testing_2020].
