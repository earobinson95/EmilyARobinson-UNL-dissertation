...
```{r 91divoc-cases-july2021, fig.scap = "91-DIVOC new daily case counts as of July 2021", fig.cap = "New daily COVID-19 case counts as of July 2021 shown in the 91-DIVOC dashboard.", out.width="75%"}
knitr::include_graphics("images/91dovic-cases-july2021.png")
```

```{r covid19-summer2020-risk-map,  fig.scap = "COVID-19 risk level map as of July 2020", fig.cap = "COVID-19 risk level map as of July 2020.", out.width="75%"}
knitr::include_graphics("images/covid19-summer2020-risk-map.png")
```

```{r covid19-datawrapper-views-july2020, fig.scap = "Datawrapper daily chart views during COVID-19", fig.cap = "Datawrapper daily chart views during COVID-19.", out.width="75%"}
...
Misleading charts might have (1) bad form such as 3D pie charts or a plot type that is unsuitable for the type of data \pcref{fig:misleading-graphics-form} [@reddit_dataisugly; @lowndes2017our], (2) include too much "chartjunk", resulting in clutter or displaying useless data \pcref{fig:misleading-graphics-chartjunk} [@graphcrimes_2022; @dongarra1997top500], or (3) have bad axes such as a mismatch between scale and context or just plain bad math \pcref{fig:misleading-graphics-bad-axes} [@villa_2022; @lauren_2018].
...
```{r misleading-graphics-form, fig.show = 'hold', fig.align='center', out.width = '49%', fig.scap = "Misleading graphical form examples", fig.cap = "These figures display information in bad form which results in an ineffective chart."}
...
```{r misleading-graphics-chartjunk, fig.show = 'hold', fig.align = 'center', out.width = '49%', fig.scap = "Misleading 'chartjunk' examples", fig.cap = "These figures provide examples of how too much clutter can be misleading."}
knitr::include_graphics(c("images/misleading-graphics-wordle-map.jpg", "images/misleading-graphics-bad-form-supercomputers.png"))
...
```{r misleading-graphics-bad-axes, fig.show = 'hold', fig.align = 'center', out.width = '49%', fig.scap = "Misleading bad axes", fig.cap = "These figures are misleading in their axis selections and bad math. (Left) Selecting a baseline $y$-axis value of 0 minimizes the large decline in the rate of the rouble in 2022. (Right) The probabilities sum to greater than 100\\%."}
knitr::include_graphics(c("images/misleading-graphics-bad-axes-rouble-to-dollar.jpg", "images/misleading-graphics-pizza.png"))
...
Many efforts have been made to provide frameworks and classification systems for graphical designs; the most useful for our purposes is Wilkinson's Grammar of Graphics [@wilkinson2013grammar].
...
@wickham2013graphical gives a review and critique of the first formal advice for creating good graphics presented by The International Institute of Statistics in 1901 and @andrews_2022 recalls seventeen general suggestions for the visualization of statistical and quantitative data published by The American Society of Mechanical Engineers (ASME) in 1915.
...
Almost 100 years later, @cleveland1988shape again makes the recommendation to "bank to 45$^{\circ}$" and @heer2006multi explores extensions to propose alternate optimization criteria as well as introduce a technique meant to implement the original guideline.
...
For instance, @amer2005bias demonstrates that visual illusion may bias decision making and graph comprehension, even if the graphs are constructed according to best practice.
...
```{r log-scales, fig.scap = "Linear scale verses log scale", fig.cap = "These plots present an exponential curve displayed on both the linear and log scale and illustrate the use of the log scale when displaying data which spans several magnitudes.", out.width="100%"}
...
Known as *Weber's law*, it was established that we do not notice absolute changes in stimuli, but instead that we notice the relative change [@fechner1860elemente].
The *Weber-Fechner law* extended the discovery and states the relationship between the perceived intensity is logarithmic to the stimulus intensity when observed above a minimal threshold of perception.
...
The hierarchy of plot objects found in @cleveland_graphical_1985 can provide a possible explanation for the underestimation that occurs in exponentially increasing trends; the exponential trend can be thought of as a series of tangential angles leading to less accurate judgement of the next points.
